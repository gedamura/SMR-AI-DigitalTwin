...
# Clear any leftover raw statepoint.*.h5 files from earlier runs (e.g. the
# Day 51 validation runs) before starting. Without this, a stale file from
# a previous batches= setting sits alongside the fresh one and the
# "exactly one file" check below can't tell them apart.
leftover = glob.glob("statepoint.*.h5")
if leftover:
    print(f"Removing {len(leftover)} leftover statepoint file(s) from a previous run: {leftover}")
    for f in leftover:
        os.remove(f)
